/**
 * 离线补审：给「审核门关着生成」的课补一份有据率读数。
 *
 * ## 为什么要它
 *
 * 消融爬升的档 0（裸生成）与档 1（+模板池）都关了 `AUDIT_GATE`，落盘的课
 * 没有 `scene.audit`。于是爬升表最左边两档**没有有据率可比**——
...
async function main() {
  ...
  for (const id of IDS) {
    ...
    console.log(
      `\n  断言 ${claims}｜支持 ${supported}｜存疑 ${uncertain}｜判错 ${incorrect}` +
        `｜有据率 ${rate == null ? '—' : `${(rate * 100).toFixed(1)}%`}`,
    );
    rows.push({
  ...
  const md =
    `# 离线补审（无证据臂）${stamp()}\n\n` +
    '给审核门关着生成的课补有据率读数。**生成条件不变，只补量具。**\n\n' +
    '有据率 = 有据 / 断言，**存疑全不计分**。不叫「事实性」是因为引擎体检（⑦ 站）\n' +
    '用的是另一套：「(有据 + 0.6×存疑) / 断言」，存疑六折计入。\n' +
    '两套差随存疑率放大（实测 5.3 与 25.0 个百分点），同名会被拿去并表。\n\n' +
