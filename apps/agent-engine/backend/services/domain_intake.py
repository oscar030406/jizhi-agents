...
def _scope_label(corpus: str, scope: str) -> str:
    ...
    **只管上屏那一格。** `scope` 还有别的消费方——⑥ 站的试跑课题名
    （`_stage_trial` 里 `subject = run.record.get("scope")`）用的是**完整原文**，
    刻意不截：课题名要说清这门课讲什么，「智能制造技能培训：ROS2 机器人开发与
    西门子 S7-1200 PLC 编程：数学基础与…」虽然长，但比截断准确。
    改这里的截断规则不会影响那边，**反过来也别以为改了这里到处都短了**。
