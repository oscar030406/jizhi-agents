...
/** Minimal shape the audit step needs — avoids importing the server-side module. */
...
export function StepVisualizer({
  ...
  lastAudit?: SceneAuditSummary | null;
}) {
  switch (stepId) {
    case 'pdf-analysis':
    ...
    case 'audit':
      return <AuditVisualizer audit={lastAudit} />;
    case 'actions':
...
/**
 * Hallucination audit: a shield scanning claim lines, verdict marks landing one
 * by one. Claim text is never faked — the bars are abstract, and only real
 * counts from a completed audit are shown as numbers.
 */
function AuditVisualizer({ audit }: { audit?: SceneAuditSummary | null }) {
  const [checked, setChecked] = useState(0);
  const rows = 5;

  useEffect(() => {
    const timer = setInterval(() => {
      setChecked((prev) => (prev + 1) % (rows + 1));
    }, 700);
    return () => clearInterval(timer);
  }, []);

  return (
    <div className="size-40 relative flex flex-col items-center justify-center gap-3">
      <motion.div
        className="absolute inset-4 bg-emerald-500/5 rounded-3xl blur-xl"
        animate={{ opacity: [0.3, 0.7, 0.3] }}
        transition={{ duration: 2.4, repeat: Infinity }}
      />

      {/* Claim lines being verified */}
      <div className="relative w-32 space-y-1.5">
        {Array.from({ length: rows }).map((_, i) => {
          const done = i < checked;
          // Every 4th row lands as "uncertain" so the amber state is visible too.
          const uncertain = done && i % 4 === 3;
          return (
            <div key={i} className="flex items-center gap-2">
              <motion.div
                ...
                animate={done ? {} : { opacity: [0.4, 1, 0.4] }}
              ...
              <motion.span
                ...
                className={cn(
                  'text-[11px] font-bold w-3 shrink-0',
                  uncertain ? 'text-amber-500' : 'text-emerald-500',
                )}
              >
                {uncertain ? '?' : '✓'}
              </motion.span>
        ...
        {/* Scanning beam */}
        <motion.div
          className="absolute inset-x-0 h-[2px] bg-gradient-to-r from-transparent via-emerald-400 to-transparent shadow-[0_0_12px_rgba(52,211,153,0.6)]"
          animate={{ top: ['0%', '95%', '0%'] }}
      ...
      {/* Real numbers from the previous scene's audit — never fabricated */}
      {audit && (
        <motion.p
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: 1, y: 0 }}
          className="absolute -bottom-1 text-[10px] text-muted-foreground whitespace-nowrap"
        >
          上一场景：{audit.totalClaims} 条断言 / {audit.flaggedCount} 条标记
        </motion.p>
      )}
    </div>
  );
