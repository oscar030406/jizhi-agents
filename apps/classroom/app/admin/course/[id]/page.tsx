...
export default async function AdminCoursePage({
  ...
  if (!course) notFound();
  const [org, ownership] = await Promise.all([orgForAccount(account.id), corpusOwnership()]);
  if (!courseVisibleToOrg(course, org?.id ?? null, ownership)) notFound();

  const scenes = (course.scenes ?? []).filter((s) => (s as { audit?: SceneAudit }).audit) as Array<{
    title?: string;
    audit: SceneAudit;
  }>;
                ...
                >
                  ...
                  <dl className="mb-5 grid grid-cols-2 gap-x-4 gap-y-4 sm:grid-cols-4">
                    {[
                      ...
                      [
                        '轮次 / 耗时',
                        `${a.rounds ?? 0} 轮 · ${Math.round((a.durationMs ?? 0) / 1000)}s`,
                      ],
                    ].map(([label, value]) => (
                      <div key={label}>
                        ...
                        {(a.debate ?? []).map((d, j) => (
                          <li key={j} className="rounded-lg border border-border px-3 py-2.5">
                            ...
                            {(d.judgeVerdicts ?? []).map((v, k) => (
                              <p
                                key={k}
                                className="mt-1.5 font-mono text-[11px] leading-relaxed text-muted-foreground"
                              >
