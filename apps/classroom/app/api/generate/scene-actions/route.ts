...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    usageAttribution.enterWith({ classroomId: stageId });
    ...
    // 生成期概念标签落到场景上。scene-content 把它算好挂在回传的 effectiveOutline 上，
    // 客户端原样转发过来——这里只是搬运，不重算。检索当时没给证据就没有这个字段，
    // 场景也就不写（存量课同样没有，读取方一律容缺）。
    if (outline.concepts) {
      scene.concepts = outline.concepts;
    }
