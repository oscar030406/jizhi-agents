...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // domain) and it is appended here so the OUTLINE itself is profile-aware —
    // scene count, staging and types differ per learner, not just wording.
    const learnerPlan = requirements.learnerProfile
      ? await fetchLearnerBlueprint(requirements.requirement ?? '', requirements.learnerProfile)
      : null;
    if (learnerPlan) {
      log.info(
        `Outline adapted: type=${learnerPlan.blueprint?.learner_type} ` +
          `difficulty=${learnerPlan.recommended_difficulty} weak=${learnerPlan.weak_concepts.slice(0, 3).join(',')}`,
      );
    }

    const userProfileText =
      requirements.userNickname || requirements.userBio || learnerPlan
        ? `## Student Profile\n\nStudent: ${requirements.userNickname || 'Unknown'}${requirements.userBio ? ` — ${requirements.userBio}` : ''}\n\nConsider this student's background when designing the course. Adapt difficulty, examples, and teaching approach accordingly.${
            learnerPlan
              ? blueprintDirective(learnerPlan, requirements.learnerProfile!) +
                `\n课程编排要求：难度档 ${learnerPlan.recommended_difficulty} 的学习者，` +
                `场景序列必须覆盖三种资源形态——讲解型（slide）、实操型（interactive/procedural）、` +
                `测验型（quiz），且测验难度与上述档位一致。薄弱概念要单独成场景补足。`
              : ''
          }\n\n---`
        : '';
