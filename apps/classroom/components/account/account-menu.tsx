...
/**
 * 账户入口：未登录显示「登录 / 注册」，登录后显示用户名与菜单（控制台 / 退出）。
 *
...
import { UserRound, LogOut, LayoutDashboard, Loader2, GraduationCap, ClipboardList } from 'lucide-react';
...
const ROLE_CARDS: ReadonlyArray<{
  ...
  { value: 'learner', label: '学习者', hint: '生成属于自己的课，按画像适配难度', Icon: GraduationCap },
  { value: 'manager', label: '管理者', hint: '接入领域语料，看班级学情与资源匹配度', Icon: ClipboardList },
];
            ...
            {!role ? (
              <div>
                ...
                <p className="mt-2.5 text-[10px] leading-relaxed text-muted-foreground">
                  两种身份共用一套账号体系，但进入的是不同的工作台。注册时选定，登录时对账。
                </p>
              </div>
            ) : (
              <>
