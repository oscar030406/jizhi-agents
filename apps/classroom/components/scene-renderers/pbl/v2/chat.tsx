  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
          ...
          if (ev.kind === 'task') {
            ...
            return (
              <div key={ev.id} className="flex justify-start">
                <div className="pbl-v2-task-review-shell max-w-[90%] rounded-[22px] px-4 py-3 text-sm">
      ...
      >
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-cyan-100/[0.13] bg-slate-700/[0.26] px-3 py-2 transition-all focus-within:border-primary/70">
...
function SubmissionEvaluationBubble({
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[90%] rounded-xl border border-primary/30 bg-primary/[0.12] px-4 py-3 text-sm shadow-card">
...
function StreamingDraft({
  ...
  if (status === 'eval-task') {
    return (
      <div className="flex justify-start">
        <div className="pbl-v2-task-review-shell max-w-[90%] animate-in fade-in-0 rounded-[22px] px-4 py-3 text-sm duration-300">
  ...
  if (status === 'eval-final') {
    return (
      <div className="space-y-3">
        <div className="animate-in fade-in-0 rounded-xl border border-border bg-card px-4 py-3 text-sm text-card-foreground shadow-card duration-300">
  ...
  if (message.roleType === 'simulator') {
    ...
    return (
      <div className="flex justify-start">
        ...
        <div className="ml-2 max-w-[85%] rounded-2xl border border-yellow-deep/30 bg-card px-4 py-3 text-sm text-card-foreground shadow-card">
          <div className="mb-1 text-[10px] uppercase tracking-wider text-yellow-deep">{name}</div>
  ...
  if (
    ...
    return (
      <div className="flex justify-center py-1">
        <div className="rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground shadow-card">
          {label}
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center py-3" aria-label={label}>
        <div className="flex min-w-[52%] max-w-[90%] items-center gap-3">
          ...
          <div className="relative rounded-full border border-purple-deep/30 bg-purple-soft px-4 py-1.5 text-[11px] font-semibold text-purple-deep shadow-card">
            <span className="absolute -left-1 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-purple-deep" />
            <span className="absolute -right-1 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-blue-deep" />
            {label}
          </div>
...
function InstructorMessageBubble({
  ...
  return (
    <div className="flex justify-start">
      <InstructorAvatar agentName={agentName} agentIntro={agentIntro} />
      <div className="pbl-v2-instructor-bubble ml-2 max-w-[85%] rounded-2xl px-4 py-3 text-sm">
...
const INSTRUCTOR_AVATAR_RING =
  'h-8 w-8 overflow-hidden rounded-full border border-primary/35 bg-primary/15';
...
function CharacterAvatar({
  ...
  if (character?.avatar) {
    return (
      <div
        className="mt-0.5 h-8 w-8 shrink-0 overflow-hidden rounded-full border border-yellow-deep/40 bg-yellow-deep/15"
        title={name}
        aria-hidden="true"
      >
  ...
  return (
    <div
      className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-yellow-deep/40 bg-yellow-deep/15 text-xs font-semibold text-yellow-deep"
      title={name}
      aria-hidden="true"
    >
...
function ThinkingDots({ tone = 'muted' }: { readonly tone?: 'muted' | 'violet' } = {}) {
  const dotClass = tone === 'violet' ? 'bg-purple-deep' : 'bg-muted-foreground';
  return (
