...
import { FileText, HelpCircle, Gamepad2, Puzzle, ShieldCheck } from 'lucide-react';
...
import { claimCourseCelebration } from '@/lib/celebration/course-celebration';
...
interface ClassroomCompletePageProps {
  ...
  /** 课程 id：L3 庆祝的防重放键。缺省时不播庆祝，只渲染静态成果页。 */
...
export function ClassroomCompletePage({ scenes, title, stageId }: ClassroomCompletePageProps) {
  ...
  // L3 整课庆祝（规格3.2.5②）：每课只播一次（状态机见 lib/celebration），
  ...
  return (
    <MotionConfig reducedMotion={prefersReducedMotion ? 'always' : 'user'}>
      ...
      >
        ...
        />
        {/* Confetti — 只在一次性庆祝窗口内燃放（每课一次，防重放） */}
        {celebrating && <Confetti />}

        {/* 跳过庆祝（规格3.2.5：2–3s 可跳过） */}
        {celebrating && (
          <button
            type="button"
            onClick={() => setCelebrating(false)}
            className="absolute top-4 right-4 z-10 rounded-full border border-border bg-card/90 px-3 py-1 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
          >
            {t('common.skip')}
          </button>
        )}
        ...
        <div className="relative flex flex-col items-center gap-6 max-w-2xl w-full px-8 py-10">
          ...
          <div className="relative" style={{ width: 200, height: 200 }}>
            <motion.div
              ...
              animate={
                celebrating
                  ? { scale: [0.9, 1.15, 0.95, 1.1], opacity: [0, 0.55, 0.4, 0.5] }
                  : { scale: 1, opacity: 0.4 }
              }
              transition={
                celebrating
            ...
            >
              <motion.div
                animate={celebrating ? { y: [0, -5, 0] } : { y: 0 }}
                transition={
                  celebrating
            ...
            {/* 循环 sparkle 只属于庆祝窗口——循环动画不当常驻装饰（反面清单⑩） */}
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 text-white text-xs font-bold uppercase tracking-wider shadow-card"
          >
          ...
          {/* 审核账单：本课交付内容的幻觉核验汇总（数据全来自各场景 audit，
              纯前端聚合——按课统计的真数，不冒充全量评测口径） */}
          <AuditBillCard scenes={scenes} />
          ...
          {summary.quiz && (
            <motion.div
              ...
              className="w-full rounded-2xl bg-gradient-to-br from-amber-100 via-orange-50 to-amber-100 dark:from-amber-950/50 dark:via-orange-950/30 dark:to-amber-950/50 border border-amber-200 dark:border-amber-900/50 px-6 py-5 shadow-card"
            >
...
/**
 * 审核账单卡：本课事实断言的核验汇总与交付端幻觉率。
 *
 * 口径与评测协议一致：幻觉=判定 incorrect（unsupported）的断言；
 * uncertain 不计入幻觉。分母=全课经审断言总数。每个场景的 audit
 * 已含内容审＋讲稿审的合并结果（mergeAudits），这里只做求和。
 * 没有任何场景经审（引擎桥全程离线）时不渲染——不编造零。
 */
function AuditBillCard({ scenes }: { readonly scenes: Scene[] }) {
  const bill = useMemo(() => {
    const audited = scenes.filter((s) => s.audit && s.audit.totalClaims >= 0);
    if (audited.length === 0) return null;
    const total = audited.reduce((a, s) => a + (s.audit?.totalClaims ?? 0), 0);
    const incorrect = audited.reduce((a, s) => a + (s.audit?.incorrectCount ?? 0), 0);
    const uncertain = audited.reduce((a, s) => a + (s.audit?.uncertainCount ?? 0), 0);
    const revised = audited.filter((s) => s.audit?.verdict === 'revised').length;
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 14, scale: 0.95 }}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ delay: 1.1, type: 'spring', stiffness: 220, damping: 20 }}
      className="w-full rounded-2xl bg-white/90 dark:bg-gray-900/70 border border-emerald-200 dark:border-emerald-900/50 px-6 py-5 shadow-card backdrop-blur-sm"
    >
