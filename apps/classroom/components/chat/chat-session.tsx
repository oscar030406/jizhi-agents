  ...
  return (
    <div
      className={cn(
        ...
        // AI 消息统一 blue-soft 底标识（规格3.2.7⑦③）；用户消息保持实心紫
        isUser
          ? 'bg-primary text-primary-foreground rounded-tr-sm'
          : isTeacher
            ? 'bg-blue-soft text-foreground border border-blue-deep/20 rounded-tl-sm'
            : 'bg-blue-soft/60 text-foreground border border-blue-deep/15 rounded-tl-sm',
      )}
    >
  ...
  if (session.messages.length === 0 && !isActive) {
    return (
      <div className="h-20 flex items-center justify-center text-center px-2">
        <p className="text-[10px] text-gray-500 dark:text-gray-400">{t('chat.noMessages')}</p>
      </div>
    );
  }
        ...
        <AnimatePresence>
          {isEnded && (
            ...
            >
              ...
              <span className="flex items-center gap-1 text-[9px] text-gray-500 dark:text-gray-400 font-medium">
            ...
            {isSoftClosing && onContinueSession && (
              ...
              >
                ...
                {remainingSoftCloseSeconds !== undefined && (
                  <span className="text-[9px] font-medium tabular-nums text-gray-500 dark:text-gray-400">
