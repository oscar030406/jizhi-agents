      ...
      {sessions.map((session) => {
        ...
        return (
          <div
            key={session.id}
            className={cn(
              // 原来是 transition-all 500ms：会话卡切到「进行中」时描边和底色慢半秒才跟上。
              // 只过渡真正会变的两个属性，时长回到站内主力档 200ms。
          ...
          >
            ...
            >
              ...
              <span className="text-[9px] text-gray-500 dark:text-gray-400 font-medium tabular-nums shrink-0">
                {session.messages.length}
              </span>
              <ChevronDown
                className={cn(
                  'w-3.5 h-3.5 text-gray-500 dark:text-gray-400 transition-transform duration-200 shrink-0',
