...
interface LectureNotesViewProps {
  ...
  /** 导学单栏：内嵌到当前小节组末尾的动态区（讨论流/邀请卡/导师考核）。
      无当前小节（生成中/未开始）时渲染在整个流末尾。 */
  currentSceneExtras?: React.ReactNode;
}
...
export function LectureNotesView({
  ...
  // Empty state（有 extras 时仍渲染 extras——讨论邀请/导师考核不依赖讲解流存在）
  if (notes.length === 0) {
    return (
  ...
  const hasCurrentGroup = !!currentSceneId && notes.some((n) => n.sceneId === currentSceneId);
      ...
      {notes.map((note, index) => {
        ...
        return (
          ...
          >
              ...
              {(() => {
                ...
                return rows.map((row, i) => {
                  ...
                  if (isSpeech) {
                    return (
                      <button
                        ...
                        className={cn(
                          // 12px 是学习者第二长时间盯的正文（第一是主画布讲义），
                          // 实测 320px 栏宽下 12px 一行约 21 个汉字。抬到 13px 换到
                          // 一行 19 字，行距按中文取 1.8（汉字字面大、没有升降部留白）。
                          'block w-full text-left rounded-md px-1 py-0.5 text-[13px] leading-[1.8] transition-colors',
                  ...
                  return (
                    <p
                      key={i}
                      className="text-[13px] leading-[1.8] text-gray-700 dark:text-gray-300"
                    >
                      {content}
                    </p>
                  );
            ...
            {/* 导学动态区：内嵌当前小节末（讨论/邀请卡/导师考核随讲义推进） */}
            {isCurrent && currentSceneExtras}
          </div>
        );
      })}
      {/* 当前小节还没进讲解流（生成中/交互场景无讲解）时，动态区落在流末尾 */}
      {!hasCurrentGroup && currentSceneExtras}
    </div>
  );
}
