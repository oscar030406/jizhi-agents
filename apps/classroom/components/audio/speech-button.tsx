...
export function SpeechButton({
  ...
  // 语音搁置或 ASR 不可用时整个按钮不渲染（2026-08-04 用户裁决）。
  // 原来是渲染成禁用态——线上实测「点了毫无反应」，一个永远点不动的图标
  // 比没有这个图标更糟。正在录音时不隐藏，否则用户没法点停（录音器无自动停止）。
  if ((!isVoiceEnabled() || !asrAvailable) && !active) return null;
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          ...
          {isRecording && (
            <span
              className="absolute inset-[-4px] rounded-[10px] border border-purple-deep/40"
              style={{
