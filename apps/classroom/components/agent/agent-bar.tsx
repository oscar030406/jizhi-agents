...
import { isVoiceEnabled } from '@/lib/config/feature-flags';
...
function AgentVoicePill({
  ...
  // TTS 关闭或无可用服务商 ⇒ 整个音色选择器不渲染。
  //
  // 上游原本渲染成禁用态（#665，理由是"不要静默隐藏控件"）——那个取舍成立于
  // "语音是产品能力、只是暂时没配"的前提。2026-08-04 用户裁决语音模块整体搁置，
  // 前提不再成立：本部署根本不提供语音，六个角色一排「无可用音色」是在展示
  // 一个不存在的功能（线上实测这是首页最扎眼的一处）。语音回归时删掉这个早退即可。
  if (disabled || !isVoiceEnabled()) return null;
...
export function AgentBar() {
    ...
    <div className="flex items-center gap-1.5 shrink-0">
      ...
      {agentMode === 'auto' ? (
        <>
          ...
          <Shuffle className="size-4 text-purple-deep" />
        </>
      ) : (
        <>
  ...
  return (
    // 手机端修复：文字标签 sm 以下 display:none 后，固定 w-96（384px）会把 390px
    // 视口顶出横向滚动条——移动端收缩为内容宽（头像排 + 箭头），sm 起恢复固定宽。
    <div ref={containerRef} className="relative w-auto sm:w-96">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="rounded-2xl bg-popover ring-1 ring-black/[0.04] dark:ring-white/[0.06] shadow-dropdown px-2 py-1.5">
              ...
              {agentMode === 'preset' ? (
                ...
                <div className="flex flex-col items-center pt-6 pb-3 gap-4">
                  <div className="relative flex items-center justify-center">
                    <div className="absolute size-10 rounded-full bg-purple-deep/10" />
                    <div className="absolute size-12 rounded-full bg-purple-deep/5" />
                    <Shuffle className="relative size-5 text-purple-deep" />
                  </div>
