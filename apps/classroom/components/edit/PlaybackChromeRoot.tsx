...
/**
 * 「用户自己动过场景侧栏」的标记。`sidebarCollapsed` 本身记的是**状态**，
 * 记不住这个状态是默认给的还是人选的——两者混在一起，默认值就只能二选一：
 * 要么所有人进来都是收起（现状：1440/1600/1920 三档实测都收起，演示要先手点开），
 * 要么强行展开，把手动收起过的人每次刷新都顶回展开。
 * 分成两个值就都能满足：没动过按视口宽度给默认，动过就一切照人选的来。
 * 存法与首页最近课堂的折叠状态一致（`app/page.tsx` 的 RECENT_OPEN_STORAGE_KEY）。
 */
const SIDEBAR_USER_SET_KEY = 'classroomSidebarUserSet';
/** 展开侧栏的视口下限（Tailwind xl）。窄屏维持收起——侧栏会吃掉画布。 */
const SIDEBAR_AUTO_OPEN_MIN_WIDTH = 1280;

/** 侧栏默认要不要展开。抽出来是为了能不起浏览器就把这两条分支跑一遍。 */
export function shouldAutoOpenSidebar(userSetFlag: string | null, viewportWidth: number): boolean {
  if (userSetFlag === '1') return false;
...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot({ onRetryOutline, canEnterProMode, onEnterProMode }, ref) {
    ...
    /** 用户亲手收/放侧栏走这个，顺手记下「这是人选的」。 */
    const chooseSidebarCollapsed = useCallback(
      (collapsed: boolean) => {
        try {
          localStorage.setItem(SIDEBAR_USER_SET_KEY, '1');
        } catch {
          /* localStorage 不可用（隐私模式）：记不住就退回每次按视口给默认 */
        }
        setSidebarCollapsed(collapsed);
      },
      [setSidebarCollapsed],
    );
    // 没被人动过的默认：宽屏展开。挂载后才读 window——SSR 没有视口宽度。
    useEffect(() => {
      let userSetFlag: string | null = null;
      try {
        userSetFlag = localStorage.getItem(SIDEBAR_USER_SET_KEY);
      } catch {
        /* 读不到就按没动过处理 */
      }
      if (shouldAutoOpenSidebar(userSetFlag, window.innerWidth)) setSidebarCollapsed(false);
    }, [setSidebarCollapsed]);
    ...
    useEffect(() => {
      const onKeyDown = (event: KeyboardEvent) => {
        ...
        switch (event.key) {
          ...
          case 'S':
            event.preventDefault();
            chooseSidebarCollapsed(!sidebarCollapsed);
            break;
          case 'c':
          case 'C':
            event.preventDefault();
    ...
    // Scene viewer height — header is 56px (h-14, 规格2.4 统一 header) when
    // visible. 底部圆桌条已撤（2026-08-03 导学大一统：对话/输入迁入右栏导学，
    // 正文下方清爽），只有放映模式保留 Roundtable 的 overlay dock（不占流高）。
    const sceneViewerHeight = (() => {
      const headerHeight = isPresenting ? 0 : 56;
      return `calc(100% - ${headerHeight}px)`;
    })();

    // 导学栏输入交互（原圆桌输入的迁入，放映模式的 Roundtable 复用同一套）
    const handleRailInputActivate = () => {
    ...
    const handleRailMessageSend = async (msg: string) => {
      // Always clear Level-1 pause state — the closure may hold a stale
      // isDiscussionPaused value.
      setIsDiscussionPaused(false);
      ...
      setChatIsSoftClosing(false);
      // User interrupts during playback — handleUserInterrupt already routes to
      // sendMessage; include 'paused' because onInputActivate pauses the engine.
      if (
        engineRef.current &&
    ...
    return (
      ...
      >
        <SceneSidebar
          ...
          onCollapseChange={chooseSidebarCollapsed}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
          ...
          >
            <CanvasArea
              ...
              onToggleSidebar={() => chooseSidebarCollapsed(!sidebarCollapsed)}
              ...
              onRequestSceneSwitch={gatedSceneSwitch}
            />
          </div>

          {/* Roundtable Area — 仅放映模式保留（overlay dock 承载全屏控制）；
              常规播放的对话/输入已迁入右栏导学（2026-08-03 导学大一统） */}
          {mode === 'playback' && isPresenting && (
            ...
            >
              <Roundtable
                ...
                onMessageSend={handleRailMessageSend}
                onDiscussionStart={() => {
                ...
                onInputActivate={handleRailInputActivate}
        ...
        <div className="flex shrink-0">
          <ChatArea
            ...
            // ── 导学大一统：对话输入/讨论邀请/思考态迁入右栏 ──
            onMessageSend={(msg) => void handleRailMessageSend(msg)}
            onInputActivate={handleRailInputActivate}
            discussionRequest={discussionRequest}
        ...
        >
          <AlertDialogContent
            ...
            className="max-w-sm rounded-2xl p-0 overflow-hidden border-0 shadow-dropdown"
          >
            <VisuallyHidden.Root>
              <AlertDialogTitle>{t('stage.confirmSwitchTitle')}</AlertDialogTitle>
            </VisuallyHidden.Root>
            {/* Top accent bar — 待确认属「进行中」语义，黄系（规格③） */}
            <div className="h-1 bg-yellow-deep/60" />

            <div className="px-6 pt-5 pb-2 flex flex-col items-center text-center">
              {/* Icon */}
              <div className="w-12 h-12 rounded-full bg-yellow-soft flex items-center justify-center mb-4 ring-1 ring-yellow-deep/20">
                <AlertTriangle className="w-6 h-6 text-yellow-deep" />
              </div>
              {/* Title */}
            ...
            <AlertDialogFooter className="px-6 pb-5 pt-3 flex-row gap-3">
              ...
              <AlertDialogAction
                ...
                className="flex-1 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground border-0"
              >
                {t('common.confirm')}
              </AlertDialogAction>
            </AlertDialogFooter>
