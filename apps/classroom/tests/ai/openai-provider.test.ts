...
describe('OpenAI provider defaults', () => {
  ...
  // WO-N10：AI SDK 的 model.provider 对每个 OpenAI 兼容第三方都是 'openai.chat'，
  // 账本前缀归一化后把 siliconflow 全记成了 openai。落账读的是这个标记，不是 SDK 的。
  it('把配置的真实 providerId 挂在模型上（OpenAI 兼容第三方不再被记成 openai）', () => {
    const compat = getModel({
      providerId: 'siliconflow',
      modelId: 'Qwen/Qwen3.5-397B-A17B',
      apiKey: 'sk-test',
    });
    expect(getConfiguredProviderId(compat.model)).toBe('siliconflow');

    const native = getModel({ providerId: 'openai', modelId: 'gpt-5.5', apiKey: 'sk-test' });
    expect(getConfiguredProviderId(native.model)).toBe('openai');
  });
