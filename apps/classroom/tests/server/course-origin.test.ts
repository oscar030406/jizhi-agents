...
describe('课程归属的判据优先级', () => {
  ...
  it('只记了 domain 没记 corpus 时用 domain', () => {
    expect(courseDomainOf({ ...empty, stage: { origin: { domain: 'mfg' } } } as never)).toBe('mfg');
  });

  it('没有出身时退回服务端生成记录', () => {
    expect(
      courseDomainOf({ ...empty, generation: { profile: { corpus: 'iotdb' } } } as never),
    ).toBe('iotdb');
  });
  ...
  it('两者都没有时判 unknown——不冒充主域', () => {
    ...
    expect(courseDomainOf(empty as never)).toBe('unknown');
  });

  it('em 前缀归主库（先导语料早已合入主索引）', () => {
    const cited = {
      ...empty,
      scenes: [{ audit: { sources: [{ source_id: 'em1#s2' }] } }],
    };
    expect(courseDomainOf(cited as never)).toBe('ai');
  });
});
