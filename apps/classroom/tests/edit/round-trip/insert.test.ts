...
describe('round-trip: element.add inserts (PR2 gate)', () => {
  ...
  // 这条测的前提就是「远程图取不到」，所以不该真发网络请求。
  // 原来它依赖 fetch 自己失败——本机走代理时 example.com 要好几秒才失败，
  // 全量跑（397 个测试文件并发）时就顶穿 5s 默认超时，间歇性变红。
  // 直接把 fetch 打桩成立即失败：语义与注释里写的一致，且不再看网络脸色。
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it('(b) inserted default image element (remote URL) — slide XML is non-empty', async () => {
    ...
    // export. The remote URL fetch is stubbed to reject, so the exporter
