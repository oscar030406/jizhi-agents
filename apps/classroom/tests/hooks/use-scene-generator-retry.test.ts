...
describe('browser scene generation retry wrappers', () => {
  ...
  /**
   * 职教任务引擎（实操指南形态）的闸门在服务端按 requirements.taskEngineMode 算。
   * 2026-08-13 之前这个字段在主生成循环里根本没被带上，服务端拿到 undefined，
   * allowProceduralSkill 恒为 false，大纲里的 procedural-skill 场景全被降级成 diagram
   * ——代码齐、开关开着、23 门课里 procedural 场景仍然是 0。
   * 这条钉住传输层：给了就必须原样发出去。
   */
  it('把 requirements.taskEngineMode 原样发给服务端', async () => {
    const { fetchSceneContent } = await import('@/lib/hooks/use-scene-generator');
    ...
    await fetchSceneContent(
      {
        ...
        allOutlines: [outline],
        stageId: 'stage-1',
        stageInfo: { name: 'Vocational Course' },
        requirements: { requirement: 'RAG 检索链路搭建实训', taskEngineMode: true },
      },
      undefined,
      retryOptions,
    );
    ...
    expect(body.requirements).toMatchObject({ taskEngineMode: true });
  });

  it('没给 requirements 时不凭空造一个', async () => {
    const { fetchSceneContent } = await import('@/lib/hooks/use-scene-generator');
    mockFetch.mockResolvedValue(jsonResponse(200, { success: true, content: { elements: [] } }));

    await fetchSceneContent(
      { outline, allOutlines: [outline], stageId: 'stage-1', stageInfo: { name: 'X' } },
      undefined,
      retryOptions,
    );
