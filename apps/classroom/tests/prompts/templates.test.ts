    ...
    for (const promptId of readdirSync(templatesDir)) {
      ...
      for (const file of ['system.md', 'user.md']) {
        ...
        try {
          ...
          // Match {{placeholder}} but NOT {{snippet:name}}, {{#if}}, {{/if}},
          // or {{摘录:…}} — the latter is the excerpt-placeholder syntax the
          // model is TAUGHT to emit (evidence-grounding injectExcerpts), a
          // deliberate literal, not a template variable.
          const matches = content.match(/\{\{(?!snippet:|#if |\/if|摘录)([^}]+)\}\}/g) || [];
