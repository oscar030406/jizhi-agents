...
describe('quiz runtime authoritative reads', () => {
  ...
  it('discards an empty legacy submission instead of completing an empty attempt', async () => {
    const store = makeStore();
    const runtimeDeps = deps(store, 'learner-a');
    // A legacy scene could leave `quizAnswers` / `quizResults` behind holding
    // nothing. Migrating that pair used to append a `reviewed` fact with no
    // answers and no results and complete the session — the stuck quiz island
    // seen in the field on 2026-08-11. An empty submission is not a submission.
    localStorageStub.setItem(ANSWERS_KEY_PREFIX + 'quiz-1', '{}');
    localStorageStub.setItem(RESULTS_KEY_PREFIX + 'quiz-1', '[]');
