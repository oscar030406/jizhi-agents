...
describe('quiz view runtime hydration', () => {
  ...
  it('uses a clean cover when a completed attempt recorded no answers', () => {
    // Field repro (2026-08-11): a `reviewed` fact with an empty answer map put
    // the view into the report screen with a 0/N banner, no per-question cards
    // and 重新答题 as its only control. Nothing to review — hand the learner a
    // fresh cover instead. The immutable record itself is untouched.
    expect(
      quizViewStateFromAttempt({
        sessionId: 'attempt-1',
        status: 'completed',
