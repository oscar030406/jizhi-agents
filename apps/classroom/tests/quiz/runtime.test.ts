...
/**
 * Force the Web-Locks-less branch of `withAttemptLock`.
 *
 * These cases rendezvous two "tabs" inside the same critical section, which is
 * only reachable when `navigator.locks` is absent. Node ≤23 had no `navigator.locks`
 * so the ambient environment supplied that for free; Node 24 ships one, real
 * mutual exclusion kicked in, and each rendezvous deadlocked until the 5s test
 * timeout. `runtime-read.test.ts` already stubs a lock manager *in* for the
 * mirror-image cases — state the absence just as explicitly.
 */
function stubNoWebLocks(): void {
...
describe('quiz attempt runtime persistence', () => {
  ...
  it('answers again after a completed empty attempt without losing its record', async () => {
    // The view self-heals an answer-less `reviewed` fact back to the cover, so
    // the learner's next answer arrives as a plain draft on the root id. It has
    // to open a fresh attempt rather than be swallowed by the completed one.
    const { store, deps } = makeHarness();
    const base = { stageId: 'stage-1', sceneId: 'scene-quiz', attemptId: 'attempt-stuck' };
    ...
    expect((await store.listRecords('attempt-stuck')).map((record) => record.payload)).toEqual([
      { payloadVersion: 1, phase: 'reviewed', answers: {}, results: [] },
    ]);
    await expect(
      loadQuizAttemptState({ stageId: base.stageId, sceneId: base.sceneId }, deps),
    ).resolves.toMatchObject({
      attemptId: 'attempt-stuck:retry:1',
      state: { sessionId: 'attempt-stuck:retry:1', phase: 'draft', answers: { q1: 'A' } },
    });
  });
