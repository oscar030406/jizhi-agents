...
beforeEach(() => {
  vi.clearAllMocks();
  // 声明「本环境没有 Web Locks」，与 `tests/quiz/runtime.test.ts` 同一修法。
  //
  // 这些用例大多让两个「标签页」在同一临界区里会合，那条分支**只有在
  // `navigator.locks` 缺席时才走得到**。Node ≤23 没有 `navigator.locks`，用例一直白嫖
  // 环境天然缺席；Node 24 开始它存在了，真互斥生效，每次会合都等到 5 秒超时——
  // 23 个用例集体红，产品代码一行没动过。
  //
  // 需要锁**存在**的那几个用例（本文件里两处）各自 `vi.stubGlobal` 覆盖，
  // `afterEach` 的 `unstubAllGlobals` 会还原，互不干扰。
