...
export interface EvidenceVerdict {
  ...
  because: {
    ...
    missed: string[];
    /**
     * 这条判定是怎么来的——不是要点，是过程注记（目前只有导学提示代价在用）。
     *
     * 单开一格是因为 `missed` 的语义是「漏掉的知识要点」，把「本题看了答案」
     * 塞进去会让复盘的人把它当成一个知识缺口。可选：旧证据没有这一格。
     */
    note?: string[];
  };
}
...
export interface EvidenceContext {
  ...
  difficulty?: number;
  /**
   * 这道题用到第几级提示（导学三级阶梯）。0/缺省 = 没要过提示。
   *
   * **只留痕，不参与计权。** 提示代价已经在引擎侧压过档了——顶层 verdict 与
   * mastery_estimate 都是压后的那份，这条证据的分值本来就已经反映了代价。
   * 这里再打一次折就是重复惩罚，而且会让已经对外发布的那套指标口径失真。
   * 记它是为了「这条证据是怎么来的」可回溯：≥2 说明答案有一部分是提示喂出来的。
   */
  hintsUsed?: number;
}
