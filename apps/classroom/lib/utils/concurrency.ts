...
/**
 * 建一个「至多 `limit` 个同时在跑」的执行器：每次调用返回一个 promise，
 * 超出限额的排队等位。与 {@link lazyBoundedMap} 的区别是**不要求预先知道全部输入**——
 * 适合边产边消费的流水线：上游一边串行产出，一边把产出交给它并发处理，
 * 上游不必先攒齐再开跑。
 */
export function boundedRunner<T, R>(
  limit: number,
  fn: (item: T) => Promise<R>,
): (item: T) => Promise<R> {
  const semaphore = createSemaphore(limit);
  return (item: T) => semaphore.run(() => fn(item));
}
