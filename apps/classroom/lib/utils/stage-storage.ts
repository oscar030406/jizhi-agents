...
import { clearCursor, loadCursor } from '@/lib/playback/cursor';
import {
  accessDocument,
  clearCurrentScene,
  getDocumentStore,
...
/**
 * Resume progress per stage (0..1) for the workbench 4px progress strips:
 * position of the playback cursor within the ordered scene list. Stages that
 * were never played (no cursor) are simply absent from the result — progress
 * is decorative and must never block the homepage.
 */
// ponytail: re-reads each document alongside getFirstSlideByStages' own read;
// merge into a single per-stage pass if homepage load time ever matters.
export async function getResumeProgressByStages(
  stageIds: string[],
  ...
  await Promise.all(
    stageIds.map(async (stageId) => {
      try {
        const [document, cursor] = await Promise.all([
          accessDocument(stageId).then((access) => access.document),
          loadCursor(stageId),
        ]);
        const scenes = document?.scenes ?? [];
        if (scenes.length === 0 || !cursor) return;
        const index = scenes.findIndex((scene) => scene.id === cursor.sceneId);
        if (index >= 0) result[stageId] = (index + 1) / scenes.length;
      } catch (error) {
        log.warn(`Failed to compute resume progress for stage ${stageId}:`, error);
      }
    }),
  );
  return result;
