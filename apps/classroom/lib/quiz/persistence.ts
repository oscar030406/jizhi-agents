...
function parseSubmittedState(rawA: string | null, rawR: string | null): SubmittedState {
  ...
  try {
    ...
    // An empty answer map is not a submission. Migrating one appended a
    // `reviewed` fact with nothing in it and completed the attempt, leaving the
    // learner on a report screen with no questions and only 重新答题.
    if (!isQuizAnswers(answers) || Object.keys(answers).length === 0) return null;
    if (rawR) {
