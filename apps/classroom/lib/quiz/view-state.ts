...
export function quizViewStateFromAttempt(
  ...
  if (state.phase === 'reviewed') {
    // A completed attempt that recorded no answers has nothing to review: the
    // report renders a 0/N banner and 重新答题, and every route out of it goes
    // through that one button. Self-heal to the cover. The record stays — the
    // next write rolls over into a new attempt session.
