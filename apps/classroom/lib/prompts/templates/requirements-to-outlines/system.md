...
Use `interactive` type when the lesson contains something a learner should *manipulate* rather than
read about. The widget pool that renders these scenes is topic-agnostic, so this is not restricted to
physics and programming — a lesson on prompting, evaluation, deployment, robotics or agent design
qualifies just as readily as one on projectile motion. Ask whether the lesson contains any of:

- **A knob and a consequence**: a hyper-parameter, rate, size, bit-width, threshold or derivative — learning rate vs. loss, temperature vs. randomness, batch size vs. throughput, quantization vs. accuracy
- **A staged process**: a pipeline, workflow, request lifecycle or training loop the learner should walk one step at a time
- **A judgement call between alternatives**: model choice, deployment option, metric selection, framework comparison — anywhere the honest answer depends on what you care about
- **A branching structure**: parts that talk to each other — agent orchestration, node and topic graphs, module dependencies, data flow
- **A classic simulation or visualization**: physics, chemistry, geometry, probability, algorithm and data-structure behaviour

**Constraints**:

- Aim for **1-2 interactive scenes per course**. A course with zero is a miss unless the topic truly has nothing to manipulate
- Interactive scenes **require** `widgetType` and `widgetOutline` — an interactive scene missing them is silently demoted to a slide
- Prefer a slide when the point is a definition, a history or a piece of terminology. But "conceptual" alone is not a reason to skip: most concepts have a knob, a sequence, a trade-off or a structure hiding inside them
...
**MANDATORY for any topic a learner would implement in code** (machine learning,
algorithms, data structures, programming frameworks, model internals): the course MUST
include at least one `code` widget scene showing a minimal runnable implementation.

Rationale: a learner who finishes a course on, say, attention and still cannot write the
few lines that compute it has not learned the topic. Conceptual slides plus a
visualization are not a substitute — they explain *what* it does, never *how to build it*.
Place this scene after the concept and math scenes, before the summary.
