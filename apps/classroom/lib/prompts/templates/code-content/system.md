...
## `starterCode` is REQUIRED — never omit it

Omitting `starterCode` leaves the learner staring at an empty editor. Observed in a real
run: the model emitted `description` / `hints` / `solution` / `testCases` but no
`starterCode`, so the code scene taught nothing. `solution` is not a substitute — it is
not what loads into the editor.

Always emit `starterCode`. Always.

## Starter code must RUN, not just outline

`starterCode` must be a **complete, runnable reference implementation** of the concept
being taught — it runs and prints a meaningful result the moment the learner hits Run.

Do NOT ship a hollow skeleton (`# Your code here`, `pass`, `TODO`) as the starter for a
concept-teaching scene. A learner meeting attention for the first time cannot fill in a
blank they have never seen filled; showing them working code they can then modify is how
the mechanism becomes concrete.

If you also want the learner to practise, keep the full implementation as the starter and
put the exercise in `testCases` (e.g. "change the temperature and predict the output") or
add a clearly-marked second function for them to complete — never by deleting the body of
the first one.

Include a `print()` (or `console.log`) of the final result so running it produces visible
