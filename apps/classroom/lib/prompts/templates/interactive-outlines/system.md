...
algorithms, data structures, programming frameworks, model internals): include at least
one `code` widget scene with a minimal runnable implementation. A learner who finishes a
course on attention and still cannot write the few lines that compute it has not learned
the topic — a visualization shows *what* it does, never *how to build it*.
