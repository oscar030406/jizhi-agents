...
**Rule of thumb**: If a piece of text reads like something a teacher would *say* rather than *show*, it does not belong on the slide.

### Length: two tiers, not one hard cap

- **Bullet items / labels / captions**: keep under ~20 words (~30 Chinese characters).
- **Explanatory blocks under a sub-heading**: one or two full sentences are welcome
  (~60–90 Chinese characters). A slide that only lists bare noun phrases teaches nothing —
  the learner cannot reconstruct *why* from labels alone.

A good teaching slide alternates: a sub-heading, then a sentence that actually explains it,
then the supporting bullets. Not a wall of two-word fragments.

### Every mechanism must carry its "why" — verifiably

Whenever a slide presents a formula, rule, or design choice (a scaling factor, a mask,
a normalization step, a hyperparameter), the slide must include at least ONE of:

- a **cause chain**: the concrete problem that appears without it, walked step by step
  (e.g. "large d_k → dot products grow → softmax saturates → gradients vanish → training stalls");
- a **worked micro-example with real numbers**: an actual 2×2 or 3×3 case computed out,
  showing values before and after (e.g. "scores [8.0, 0.1] → softmax [0.9996, 0.0004]");
- a **parameter-consequence note**: what concretely changes if the key parameter is doubled,
  halved, or removed.

A formula shown without any of these is a caption, not teaching. After reading the slide,
the learner must be able to **recompute or predict something**, not just recognize a term.
Prefer numbers over adjectives: "attention weights [0.85, 0.10, 0.05]" beats "weights focus
on relevant tokens". If the scene covers multiple mechanisms, every one of them needs its
"why" — do not spend the whole budget on the first and caption the rest.

### Prefer a table when the content is a comparison or a mapping

When the scene compares two or more things (old vs new, problem vs solution), or maps
items to their roles / use cases, use a **TableElement** instead of parallel bullet lists.
Tables carry more information per pixel and are easier to scan than repeated bullets.
See the TableElement spec below.
...
### Rule 9: Shapes must earn their pixels

Decorative shapes are the fastest way to make a slide feel empty. A learner reads text,
tables, and labeled diagrams — not colored blobs.

- **Ban the two-blob comparison**: never render "A vs B" as two large circles/rectangles
  with one short label each plus an arrow. That layout spends half the canvas to convey
  two nouns. Use a TableElement (rows = aspects, columns = A / B) or a labeled multi-step
  flow instead.
- **Size must match information**: a shape larger than 200×200 must contain (or directly
  back) text with at least 3 distinct facts — steps, labeled parts, or data values.
  A shape whose only content is a 2-6 word label may not exceed 200×200.
- **Diagrams need ≥ 4 labeled parts** to justify more than 25% of canvas area. A flow of
  fewer nodes should be compact (single row, modest node sizes), leaving room for real
  content — the explanation, example, or numbers that Rule "why" requires.
- When in doubt, cut the shape and spend the space on a worked example or a table.
...
- ✓ [shape-density] **No decorative blobs** (Rule 9): no two-blob comparison diagrams; every shape > 200×200 carries ≥ 3 facts; diagrams over 25% of canvas have ≥ 4 labeled parts. Comparisons use TableElement.
- ✓ [excerpt-box] Every text element whose content is a textbook-excerpt placeholder (the 「摘录:id」 double-brace token) has width ≥ 760 and height ≥ 240 — smaller boxes cause the injected excerpt to be trimmed.
