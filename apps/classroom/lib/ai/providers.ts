...
export const PROVIDERS: Record<ProviderId, ProviderConfig> = {
  ...
  siliconflow: {
    ...
    models: [
      // Qwen3 Series — generation tier for course pipelines. Unregistered models
      // resolve to modelInfo=null, which leaves maxOutputTokens unset and silently
      // truncates long payloads (a 19KB interactive-widget HTML dies mid-tag).
      {
      ...
      // 教具矩阵实测能一次产出闭合且 script 平衡 HTML 的档次。
      // 17 个候选里只有 7 个过关，这两个是其中最快的；数据与判据见
      ...
      // GLM Series — independent judge tier for the hallucination audit gate
...
/**
 * 我们配置的真实 providerId，挂在 getModel() 造出来的模型对象上。
 *
 * AI SDK 自己的 `model.provider` 是 **SDK 内部标识**：`createOpenAI()` 无论指向谁，
 * 都报 `'openai.chat'`。所有 OpenAI 兼容的第三方（siliconflow / deepseek / qwen /
 * glm…）落账时被前缀归一化成 `openai`，2026-08 账本里 6235 行 `providerId: "openai"`
 * 实际上绝大多数是 siliconflow。用量账本要的是我们配置的那个 id，不是 SDK 的。
 */
const MODEL_PROVIDER_ID = Symbol.for('openmaic.usage.providerId');

/** 读回 {@link MODEL_PROVIDER_ID}；不是 getModel() 造的模型返回 undefined。 */
export function getConfiguredProviderId(model: unknown): string | undefined {
  if (!model || typeof model !== 'object') return undefined;
  const tagged = (model as Record<symbol, unknown>)[MODEL_PROVIDER_ID];
...
function getCompatThinkingBodyParams(
  ...
  switch (capability.requestAdapter) {
    ...
    case 'siliconflow': {
      ...
      // 关思考时 pickThinkingBudget 会给 0，而硅基流动要求 thinking_budget > 0
      // （实测 400：code 20015 "must be greater than 0"）——disabled/非正预算
      // 一律不发 budget 字段。
      if (mode !== 'disabled' && budget !== undefined && budget > 0) {
...
export function getModel(config: ModelConfig): ModelWithInfo {
  ...
  // 把真实 providerId 挂回模型对象，供落账时读（见 MODEL_PROVIDER_ID）。
  // 不可枚举：不进 JSON、不进展开、不影响任何 toEqual 断言。
  Object.defineProperty(model, MODEL_PROVIDER_ID, {
