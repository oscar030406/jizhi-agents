 ...
 *
 * 最大信息那一支现在有实现了：`lib/quiz/item-selection.ts`。它是**并列**的第三条排序键，
 * 不是这里的替换，也没塞进本函数当 tie-break——测评目标（把 θ 量准）与学习目标
 * （别让他失败）最优点不同，混进来会让测评口径悄悄改掉学习路径的顺序。
 * 上面那个 0.683 与 0.75 的关系在那边钉成了断言。
 */
