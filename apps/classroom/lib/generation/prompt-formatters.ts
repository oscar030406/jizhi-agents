...
export function buildCourseContext(ctx?: SceneGenerationContext): string {
  ...
  // 全课已用过的开头。上面那段按设计只带上一页、只截尾 150 字（给衔接用），
  // 模型因此看不见自己整门课在重复什么开场。2026-08-13 实测 23 门课 557 条口播：
  // 「这一节的核心」17 次、「这一节的关键」12 次、「大家好，欢迎」23 次，
  // 前 12 个开头覆盖 22% 的条目。统计与判据见 lib/generation/speech-lint.ts 头注。
  if (ctx.usedOpenings && ctx.usedOpenings.length > 0) {
    lines.push('');
    lines.push('Openings already used in THIS course — do not reuse any of them:');
    for (const o of ctx.usedOpenings) lines.push(`  - "${o}"`);
  }
