...
import { stripExcerptPlaceholders } from '@/lib/generation/evidence-grounding';
import { expandSlideTemplate, type SlideSlotSpec } from './slide-templates';
...
import {
  ...
  formatViolations,
  ADAPTATION_REWRITE_SYSTEM,
  BROAD_TIER_SWEEP,
  type Tier,
} from './adaptation-lint';
import { DEFAULT_LANGUAGE_DIRECTIVE } from './outline-generator';
import { postProcessInteractiveHtml } from './interactive-post-processor';
import { buildTemplateCatalogText, parseTemplateSelection } from './widget-templates';
 ...
 */
/**
 * 讲义生成（slide 场景主路径，2026-08-03 讲义真形态定稿）：LLM 出讲义
 * markdown（连贯正文＋why 证据件），mdToElements 转单栏 DSL。摘录占位符
 * 独立成行进大盒，injectExcerpts / 审核门 / KR2 验算 / PPTX 导出零改动兼容。
...
/** 讲义 md（已 clean）→ GeneratedSlideContent。空/无元素返回 null。 */
export function lectureContentFromMd(
  md: string,
  outline: SceneOutline,
): GeneratedSlideContent | null {
  const slide = mdToElements(md);
  if (!slide.elements.length) return null;
  const processedElements: PPTElement[] = slide.elements.map((el) => ({
    ...el,
...
/**
 * 生成后机械 lint + 定向改写环（三档通用）。
 *
 * 取代了原来「description 带【零基础硬要求】就无条件多发一次模型自查调用」的做法。
 * 依据是 docs/03-design/adaptation-lint-spec-20260811.md §3.1：
 * ① 纯提示式自评找不出错（Kamoi TACL 2024 / Tyen et al. 2311.08516），模型强在「改」
 *    不在「找」；能用代码判的判据交给代码（IFEval 2311.07911 的设计原则）。
 * ② 校准实测 L1 场景里 10/18 是 lint 全过还硬跑那一次调用——白花的。
 * 所以现在是：先算 5 个指标定位越界点，有 A 类违规才发一次带行号与原文片段的定向改写。
 * L1/L2/L3 三档都走这条路，只是触发条件不同（规格 §3.2 规则表）。
 *
 * 挂点在这里（摘录注入**之前**）是有意的：lint 只对模型自撰区负责，看到的就该是
 * `{{摘录:id}}` 占位符。摘录区的形态上限归检索侧。阈值已按这个输入形态重新校准
 * （规格 §2.0「口径修订」），第一版是在注入后的成品上校准的，两版数字不可比。
 *
 * 上限 2 轮，第 2 轮只在第 1 轮 A 类违规数严格下降且仍非空时才发（持平或上升说明模型
 * 在原地打转，Zhang et al. 2412.14959）。所有版本作废时保留原稿，绝不静默丢内容。
 */
export async function runAdaptationLintLoop(
  original: string,
  tier: Tier | null,
  outline: SceneOutline,
  aiCall: AICallFn,
): Promise<string> {
  const tag = `[适配 lint ${tier ?? '无档位'}] ${outline.title}`;
  const first = lintAdaptation(original, tier);
  if (first.b.length) {
    // B 类不进改写 prompt：素材不够硬不是改写能解决的（走检索侧），
    // 结构告警也不该让模型顺手去动摘录
    log.warn(`${tag} B 类警告（不触发调用）：${formatViolations(first.b)}`);
  }
  if (!first.a.length) {
    log.debug(`${tag} 未触发：A 0 条，跳过改写调用`);
    return original;
  }

  // 候选 = {原稿, 第1轮, 第2轮}，选 A 类违规最少的；并列取最早的那版（改动最小）
  let best = { md: original, report: first, round: 0 };
  let cur = first;
  let curMd = original;
  for (let round = 1; round <= 2; round++) {
    let revised: string | null = null;
    try {
      revised = cleanLectureMarkdown(
        await aiCall(ADAPTATION_REWRITE_SYSTEM, buildRewriteDirective(cur, curMd)),
        outline.title,
      );
    } catch {
      log.warn(`${tag} 第 ${round} 轮改写调用失败`);
      break;
    }
    if (!revised) {
      log.warn(`${tag} 第 ${round} 轮改写产物为空`);
      break;
    }
    const integrity = checkRewriteIntegrity(original, revised);
    if (!integrity.ok) {
      log.warn(`${tag} 第 ${round} 轮完整性检查未过（${integrity.reason}）— 该版本作废`);
      break;
    }
    const next = lintAdaptation(revised, tier);
    if (next.a.length < best.report.a.length) best = { md: revised, report: next, round };
    if (next.a.length === 0 || next.a.length >= cur.a.length) break;
    cur = next;
    curMd = revised;
  }

  // 定向改写没起作用时回落到整篇审校改写（2A 复测 beginner 27.8% 的修复）：
  // 上一版把 L1 自查环整体换成 lint 环，结果 18 个 L1 场景里 7 次「未修好保留原稿」
  // ——既没自查改写也没 lint 修复，净空档。定向指令赢在精准、输在指令过窄改不动的
  // 情形；两者互补而非替代，所以定向失手就把老的整篇审校补回来当第二道。
  // 整篇审校是按档位写的口径，没有档位时这一道不适用（也没有对应的提示词）。
  if (best.round === 0 && tier) {
    try {
      const swept = cleanLectureMarkdown(await aiCall(BROAD_TIER_SWEEP[tier], curMd), outline.title);
      if (swept) {
        const integrity = checkRewriteIntegrity(original, swept);
        const after = lintAdaptation(swept, tier);
        if (integrity.ok && after.a.length < first.a.length) {
          log.info(`${tag} 定向失手→整篇审校生效：A ${first.a.length}→${after.a.length}`);
          best = { md: swept, report: after, round: 3 };
        } else {
          log.warn(
            `${tag} 定向失手→整篇审校也没救回（${integrity.ok ? `A ${first.a.length}→${after.a.length}` : integrity.reason}），保留原稿`,
          );
        }
      }
    } catch {
      log.warn(`${tag} 整篇审校调用失败，保留原稿`);
    }
  }

  if (best.round === 0) {
    log.warn(
      `${tag} 触发未修好：A ${first.a.length} 条未减少，保留原稿 — ${formatViolations(first.a)}`,
    );
  } else if (best.report.a.length === 0) {
    log.info(`${tag} 触发已修：A ${first.a.length}→0（第 ${best.round} 轮）`);
  } else {
    log.warn(
      `${tag} 触发部分修复：A ${first.a.length}→${best.report.a.length}（第 ${best.round} 轮）` +
        ` 剩余 ${formatViolations(best.report.a)}`,
    );
  }
  return best.md;
}

async function generateSlideByLecture(
  outline: SceneOutline,
  aiCall: AICallFn,
  languageDirective?: string,
): Promise<GeneratedSlideContent | null> {
  const prompts = buildLecturePrompts(outline, languageDirective);
  if (!prompts) return null;

  const response = await aiCall(prompts.system, prompts.user);
  let md = cleanLectureMarkdown(response, outline.title);
  if (!md) {
    log.warn(`Lecture markdown empty/unusable for: ${outline.title}`);
    return null;
  }
  // 蓝图分档硬指令的标记同时充当档位信号；没有画像通道（无标记）就不跑 lint。
  // 整环包一层 catch：环内除 aiCall 之外的部分（lint 本身、完整性检查）没有兜底，
  // 而讲义已经生成出来了——lint 是增强，它挂掉不该让这一页连带 return null，
  // 更不该把异常抛穿三层降级（讲义→槽位→自由版面）让整个场景重来。
  // 有档位标记就跑整套（分档阈值 + AI 味词表）；没有标记也要跑——
  // AI-TELL 与档位无关（教材在任何难度段都不写那些词），而档位标记来自引擎蓝图，
  // **引擎离线时 scenePlan 为 null、标记不注入**（lib/server/classroom-generation.ts
  // 的 fetchLearnerBlueprint 拿不到就返回 null）。按老写法，引擎一挂整篇讲义
  // 一条机械检查都不跑，恰好是最需要兜底的时候。
  const tier = tierFromDirective(outline.description);
  try {
    md = await runAdaptationLintLoop(md, tier, outline, aiCall);
  } catch (err) {
    log.warn(`[适配 lint ${tier ?? '无档位'}] ${outline.title} 环内异常，保留原稿 — ${err}`);
  }
  const content = lectureContentFromMd(md, outline);
  if (!content) return null;
  log.debug(`Lecture md → ${content.elements.length} elements for: ${outline.title}`);
  return content;
}

/** 模板槽位生成：LLM 出 {template, slots, remark}，确定性展开器排版。 */
async function generateSlideBySlots(
  outline: SceneOutline,
  aiCall: AICallFn,
  ...
  const response = await aiCall(prompts.system, prompts.user);
  const spec = parseJsonResponse<SlideSlotSpec>(response);
  if (!spec || typeof spec.template !== 'string') {
    log.warn(`Slot spec unparsable for: ${outline.title}`);
    return null;
  }
  const expanded = expandSlideTemplate(spec);
  if (!expanded) {
    log.warn(`Unknown/empty slide template "${spec.template}" for: ${outline.title}`);
    return null;
  }
  log.debug(`Slot template "${spec.template}" → ${expanded.elements.length} elements`);

  // 与自由版面路径同一后处理：latex 渲染 + 唯一 id
  const latexProcessed = processLatexElements(expanded.elements as GeneratedSlideData['elements']);
  const processedElements: PPTElement[] = latexProcessed.map((el) => ({
    ...el,
    id: `${el.type}_${nanoid(8)}`,
    rotate: 0,
  })) as PPTElement[];
  ...
  return {
    ...
    remark: spec.remark || outline.description,
  };
}
...
async function generateSlideContent(
  ...
  // ── 模板槽位路径（默认）──
  // 版式由人定、LLM 只填槽——溢出/重叠在构造上不可能（调研台账
  // docs/04-research/slide_generation_research_20260803.md）。三种情况走
  // 自由版面旧路：编辑模式（改的是既有自由 DSL）、有图片/媒体（模板未
  // 覆盖媒体版式）、槽位生成失败（回退，SLIDE_TEMPLATE_MODE=0 可全局关）。
  // ── 讲义主路径（学习者端唯一形态，2026-08-03 定稿）──
  // 连贯正文替代幻灯片短句：LLM 出讲义 md → 单栏 DSL。编辑模式（改既有
  // 自由 DSL）与含媒体场景不走（讲义 md 不承载 image/video 元素）。
  // 失败降级：讲义 → 模板槽位 → 自由版面。
  const lectureEligible =
    !editDirective &&
    !baselineContent &&
    !mediaElementEnabled &&
    process.env.LECTURE_SCENE_MODE !== '0';
  if (lectureEligible) {
    const lectureContent = await generateSlideByLecture(outline, aiCall, languageDirective);
    if (lectureContent) return lectureContent;
    log.warn(`Lecture generation failed for "${outline.title}", falling back to slot templates`);
  }
  ...
  const slotEligible =
    ...
    !baselineContent &&
    !mediaElementEnabled &&
    process.env.SLIDE_TEMPLATE_MODE !== '0';
  if (slotEligible) {
    const slotContent = await generateSlideBySlots(
      outline,
      aiCall,
      teacherContext,
      languageDirective,
    ...
    log.warn(`Slot generation failed for "${outline.title}", falling back to free-form layout`);
  }
...
async function generateQuizContent(
  ...
  // 答案键铁律（2026-08-10 排雷：两门课各有一道 answer 为空的死题带着
  // hasAnswer: true 出货，判分无从谈起）：选择题的答案必须非空且落在选项
  // value 集内。模型常见失误是把答案写成选项原文而不是字母——先按 label
  // 映射回 value 抢救；抢救不回来的题整题丢弃并记日志，绝不带死题出货。
  let droppedNoAnswer = 0;
  const questions: QuizQuestion[] = generatedQuestions.flatMap((q): QuizQuestion[] => {
    const isText = q.type === 'short_answer';
    if (isText) {
      return [{ ...q, id: q.id || `q_${nanoid(8)}`, options: undefined, answer: undefined, hasAnswer: false }];
    }
    const options = normalizeQuizOptions(q.options);
    let answer = normalizeQuizAnswer(q as unknown as Record<string, unknown>);
    if (options?.length) {
      const values = new Set(options.map((o) => o.value));
      if (answer?.length) {
        answer = answer.map((a) => {
          if (values.has(a)) return a;
          const byLabel = options.find((o) => o.label.trim() === a.trim());
          return byLabel ? byLabel.value : a;
        });
      }
      if (!answer?.length || !answer.every((a) => values.has(a))) {
        droppedNoAnswer += 1;
        return [];
      }
    } else {
    ...
    return [{ ...q, id: q.id || `q_${nanoid(8)}`, options, answer, hasAnswer: true }];
  });
...
// ==================== Template Widget Generation（模板池参数化） ====================

/**
 * 交互场景主路径：LLM 从模板池选一个模板并填参数（JSON），校验不过带错误重试一次。
 * 选不出模板 / 两次校验都失败 → 返回 null，交由上游既有降级链处理（讲义兜底）。
 * 排版与交互逻辑由站内 React 模板确定性渲染，不再让模型现场写 HTML。
 */
export async function generateTemplateWidgetContent(
  outline: SceneOutline,
  aiCall: AICallFn,
  languageDirective?: string,
  ...
  if (!prompts) {
    log.error(`Failed to build template-select prompt for: ${outline.title}`);
    return null;
  }
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const user =
      attempt === 0
        ? prompts.user
        : `${prompts.user}\n\n## Previous Attempt Rejected\n${lastError}\nFix the issues and output the corrected JSON object only.`;
    const response = await aiCall(prompts.system, user);
    const result = parseTemplateSelection(response);

    if (result.status === 'selected') {
      log.info(
        `Template widget selected for "${outline.title}": ${result.config.templateId}`,
      );
      // html 留空：模板教具由 InteractiveRenderer 按 widgetConfig 走站内组件渲染，
      // 不进 iframe；旧课程数据（html 非空）仍走原 iframe 路径。
      return { html: '', widgetType: 'template', widgetConfig: result.config };
    }
    if (result.status === 'none') {
      log.info(`No widget template fits "${outline.title}" (${result.reason}) — degrading`);
      return null;
    }
    lastError = result.error;
    log.warn(
      `Template selection attempt ${attempt + 1} invalid for "${outline.title}": ${result.error}`,
    );
  }
  return null;
}
...
export async function generateSceneActions(
  ...
  if (
    ...
    'html' in content &&
    content.widgetConfig?.type === 'template'
  ) {
    // 模板教具：DOM 由站内 React 组件确定性渲染，面向 iframe HTML 的 widget_* 动作
    // 无处落点，不再让 LLM 编排；guide 文案直接作讲解词。
    const guide = content.widgetConfig.guide;
    return [
      {
        id: `action_${nanoid(8)}`,
        type: 'speech',
        title: '交互引导',
        text: guide
          ? `现在让我们通过这个交互教具来探索概念。${guide}`
          : '现在让我们通过交互教具来探索这个概念。请尝试操作页面中的元素，观察变化。',
      },
    ];
  }
...
function processActions(actions: Action[], elements: PPTElement[], agents?: AgentInfo[]): Action[] {
  ...
  return actions.map((action) => {
    ...
    // 讲稿台词里不该出现摘录占位符——那条协议只对板书正文生效，台词没有注入环节，
    // 占位符会原样念出来。两条生成路径都经过这里，剥在这一处就够。
    const spoken = (processedAction as { text?: unknown }).text;
    if (typeof spoken === 'string') {
      (processedAction as { text?: string }).text = stripExcerptPlaceholders(spoken);
    }
