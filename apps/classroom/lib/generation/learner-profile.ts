 ...
 */
/**
 * 每档的**示例文风**：同一句话在这一档该写成什么样。
 *
 * 分档硬要求已经写了五条规则，但规则是抽象的（「术语要定义」「不做逐行注释」），
 * 模型对抽象规则的服从是表演性的——第三轮对照课那五个「本段目标：」就是
 * 照着「每段先明确目标」这条规则演出来的。**给一句成品，比给五条规则管用**：
 * 它把「写成什么样」从判断题变成对照题。
 *
 * 三档写的是**同一个知识点**（PLC 循环监视时间），差别只在语域，
 * 这样模型能看出变的是什么、不变的是什么。样例故意不带类比也不带数字表格——
 * 那两样各有自己的指令管，样例只示范语域。
 */
const REGISTER_SAMPLE: Record<'L1' | 'L2' | 'L3' | 'L4', string> = {
  L1:
    'PLC 每跑完一圈程序叫一个「扫描周期」。监视时间就是给这一圈设的闹钟：' +
    '一圈还没跑完闹钟就响了，说明程序卡住了，PLC 会直接停机保护设备。',
  L2:
    '扫描周期是 PLC 执行一轮用户程序的耗时。循环监视时间（watchdog）是这一轮的超时阈值，' +
    '超时即触发停机——作用与服务端的请求超时一致：宁可失败，不许无限等待。',
  L3:
    '看门狗阈值需覆盖最坏路径的扫描耗时，而非平均值。阈值贴着均值设会在负载尖峰上误停机，' +
    '设得过松则失去保护意义；工程取值通常留 2 倍余量，并按实测最长周期回归校正。',
  L4:
    '看门狗阈值需覆盖最坏路径的扫描耗时，而非平均值。阈值贴着均值设会在负载尖峰上误停机，' +
    '设得过松则失去保护意义；工程取值通常留 2 倍余量，并按实测最长周期回归校正。',
};
...
export function blueprintDirective(
  ...
  // 示例文风。挂在硬要求之后：先给规则再给成品，模型照着对，
  // 而不是照着五条抽象规则各自发挥（发挥出来的就是「本段目标：」那种表演）。
  const sample = REGISTER_SAMPLE[tier];
  if (sample) {
    lines.push(
      `- 【这一档的文风长这样】下面这句是同一个知识点在本档的写法，` +
        `照这个语域写（句子长度、术语密度、要不要下定义、称呼读者的方式）——` +
        `**照的是语气不是内容，不要把这句抄进正文**：
  「${sample}」`,
    );
  }
