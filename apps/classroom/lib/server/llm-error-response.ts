...
/**
 * Dig an upstream HTTP status out of an arbitrary SDK error (unwraps
 * RetryError / APICallError / `cause` chains). Exported for the generation-side
 * cross-model fallback classifier (lib/ai/model-fallback.ts) — one place decides
 * what "the upstream said 5xx" means.
 */
export function statusFromError(error: unknown, seen = new Set<unknown>()): number | undefined {
