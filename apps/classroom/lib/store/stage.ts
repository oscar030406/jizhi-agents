...
interface StageState {
  ...
  /**
   * Scenes the audit gate refused to publish, with the ruling that refused them.
   *
   * A blocked scene never enters `scenes`, so without this the block leaves no
   * trace anywhere: the classroom silently has a hole and the audit console's
   * "被拦截场景" counter — which reads from `scenes` — is structurally always 0.
   * Blocking is the load-bearing claim of the whole gate; it has to be visible.
   */
  blockedScenes: Array<{ title: string; audit: SceneAudit; at: string }>;
  ...
  recordBlockedScene: (title: string, audit: SceneAudit) => void;
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  recordBlockedScene: (title, audit) => {
    // Keyed by title+rationale so a retry of the same scene that gets blocked
    // for the same reason does not stack duplicates in the evidence list.
    const key = `${title}|${audit.rationale}`;
    if (get().blockedScenes.some((b) => `${b.title}|${b.audit.rationale}` === key)) return;
    set({
      blockedScenes: [
        ...get().blockedScenes,
        { title, audit, at: new Date().toISOString() },
      ],
    });
  },
