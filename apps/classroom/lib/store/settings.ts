...
export interface SettingsState {
  ...
  /** 场景侧栏宽度。与 sidebarCollapsed 一起持久化——折叠状态记得住、宽度记不住，
      拖完一刷新就弹回默认值，是上一版的不一致。 */
  sidebarWidth: number;
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        // TTS ON by default via the browser-native Web Speech API (no key, no
        // cost, works offline) so a fresh install lectures out loud instead of
        // silently. Upstream defaults this off and shows an enable CTA (#665);
        // for a demo-first build the spoken classroom IS the product.
        ttsEnabled: true,
        asrEnabled: true,
